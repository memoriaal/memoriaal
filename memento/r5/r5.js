...
var cmd = 'pdftotext "r_5.pdf" -nopgbrk -enc UTF-8 -f 26 -l 728 -layout'
exec(cmd, function(error, stdout, stderr) {
  var lineReader = require('readline').createInterface({
    input: require('fs').createReadStream('r_5.txt')
  })
  ...
  lineReader.on('line', function (line) {
    ...
    let page_name_re = /^( {0,113}[A-ZÕÜÄÖŠŽ\-]{3,})$/
    if (page_name_re.exec(line)) { return }

    let page_number_re = /^ {0,113}([0-9]{2,3})$/
    ...
    let line_split_re = /(^.{1,56}[^ ]*$|^.{53,} {3})(.*)/
    match = line_split_re.exec(line)
    if (match) {
      if (match[1].trim().length) {
        leftstream.write(('|' + match[1]).trim() + '\n')
      }
      if (match[2].trim().length) {
        let leftspacer = new Array(match[1].length + 1).join(' ')
        rightstream.write(leftspacer + match[2].trim() + '\n')
      }
    }
    // parse_line(line)
  })
